...
export const App = () => {
    ...
    const handleSubmit = async () => {
        if (!input.trim() || isProcessing || pendingConfirmation)
            return;
        if (pendingQuestion) {
    ...
    useInput((input, key) => {
        ...
        if (pendingConfirmation) {
            if (input === 'y' || input === 'Y') {
                ...
                pendingConfirmation.resolve('allowAll');
                setPendingConfirmation(null);
            }
        }
    });
    return (React.createElement(Box, { flexDirection: "column", padding: 1 },
        React.createElement(Box, { borderStyle: "round", borderColor: "cyan", paddingX: 1, marginBottom: 1, justifyContent: "space-between" },
            React.createElement(Text, { bold: true, color: "cyan" }, "Codeline CLI"),
            React.createElement(Text, { dimColor: true },
                "Model: ",
                currentModel)),
        React.createElement(Box, { flexDirection: "column", marginBottom: 1 }, messages.map((msg, i) => (React.createElement(Box, { key: i, flexDirection: "column", marginBottom: 1 },
            React.createElement(Box, null,
                React.createElement(Text, { bold: true, color: msg.role === 'user' ? 'green' :
            ...
            React.createElement(Box, { paddingLeft: 2 }, msg.role === 'assistant' ? (React.createElement(MarkdownView, { text: msg.content })) : (React.createElement(Text, null, msg.content))))))),
        isChoosingModel && (React.createElement(Box, { borderStyle: "double", borderColor: "yellow", flexDirection: "column", padding: 1, marginBottom: 1 },
            React.createElement(Text, { bold: true, color: "yellow" }, "Model Secin (Ok tuslari ile gezin, Enter ile secin):"),
            AVAILABLE_MODELS.map((model, i) => (React.createElement(Text, { key: model, color: i === selectedIndex ? 'cyan' : undefined },
                i === selectedIndex ? '● ' : '○ ',
                " ",
                model))))),
        pendingQuestion && (React.createElement(Box, { borderStyle: "double", borderColor: "magenta", flexDirection: "column", padding: 1, marginBottom: 1 },
            React.createElement(Text, { bold: true, color: "magenta" },
                "SORU: ",
                pendingQuestion.question),
            pendingQuestion.type === 'choice' && pendingQuestion.options?.map((option, i) => (React.createElement(Text, { key: option, color: i === selectedIndex ? 'cyan' : undefined },
                i === selectedIndex ? '● ' : '○ ',
                " ",
                option))),
            pendingQuestion.type === 'text' && (React.createElement(Box, null,
                React.createElement(Text, { color: "cyan" }, "Cevap: "),
                React.createElement(TextInput, { value: input, onChange: setInput, onSubmit: handleSubmit }))))),
        pendingConfirmation && (React.createElement(Box, { borderStyle: "double", borderColor: "red", padding: 1, marginBottom: 1, flexDirection: "column" },
            React.createElement(Text, { color: "red", bold: true },
                "ONAY GEREKL\u0130: ",
                pendingConfirmation.toolName),
            React.createElement(Text, { dimColor: true },
                "Arg\u00FCmanlar: ",
                JSON.stringify(pendingConfirmation.args).substring(0, 200),
                "..."),
            React.createElement(Box, { marginTop: 1 },
                React.createElement(Text, { bold: true, color: "green" }, "[y] \u0130zin Ver"),
                React.createElement(Text, null, " | "),
                React.createElement(Text, { bold: true, color: "red" }, "[n] Reddet"),
                React.createElement(Text, null, " | "),
                React.createElement(Text, { bold: true, color: "yellow" }, "[a] T\u00FCm\u00FCne \u0130zin Ver (Oturum Boyunca)")))),
        isProcessing && !pendingConfirmation && (React.createElement(Box, { marginBottom: 1 },
            React.createElement(Text, { color: "yellow", italic: true }, "Model calisiyor..."))),
        !pendingConfirmation && !isChoosingModel && (React.createElement(Box, null,
            React.createElement(Text, { color: "cyan" }, "Prompt: "),
            React.createElement(TextInput, { value: input, onChange: setInput, onSubmit: handleSubmit, placeholder: "Mesajinizi yazin veya /model komutunu kullanin..." }))),
        React.createElement(Box, { marginTop: 1 },
            React.createElement(Text, { dimColor: true }, "(Esc: Cikis | /model: Model Degistir)"))));
};
